...
class Sokoban {
    constructor({level}) {
        ...
        this.board = generateGameBoard({level:this.level})
    }
    ...
    render(options = {}) {
        ...
        if (isVoid(levelsMap[this.level][this.findPlayerCoords().y][this.findPlayerCoords().x])) {
            rowsWithVoid.push(levelsMap[this.level][this.findPlayerCoords().y]);
        }
        ...
        if (isWin && (this.level+1) == levelsMap.length) {
            ...
            this.context.fillStyle = colors.success_block.fill
            this.level++;
            this.context.fillText('Next Level Is ' + (this.level + 1), 65, 300);
            setTimeout(() => {
                this.render({restart: true});
            }, 3000);

            document.getElementById("level").innerHTML = "Level " + (this.level+1);
        }
    ...
    movePlayer(playerCoords, direction) {
        ...
        this.board[playerCoords.y][playerCoords.x] =
            isVoid(levelsMap[this.level][playerCoords.y][playerCoords.x]) ? VOID : EMPTY
    ...
    movePlayerAndBoxes(playerCoords, direction) {
        ...
        if (isBlock(this.board[newBoxY][newBoxX])) {
            ...
            if (isTraversible(nextBlock)) {
                for (let i = 0; i < blocksInARow; i++) {
                    ...
                    this.board[getY(newBoxY, direction, i)][getX(newBoxX, direction, i)] =
                        isVoid(levelsMap[this.level][getY(newBoxY, direction, i)][getX(newBoxX, direction, i)])
                            ? SUCCESS_BLOCK
                            : BLOCK
                }
            ...
            this.board[newBoxY][newBoxX] = isVoid(levelsMap[this.level][newBoxY][newBoxX]) ? SUCCESS_BLOCK : BLOCK
            this.movePlayer(playerCoords, direction)
        }
    }
